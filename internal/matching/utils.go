...
var TestFunctionSuffixes = []string{
	// Standard test lifecycle suffixes
	"_basic",
	"_generated",
	"_complete",
	"_update",
	...
	"_full",
	"_minimal",
	// Extended patterns for conditional and variant tests
	"_withAndWithoutCondition", // Must come before _withCondition
	"_withCondition",
	"_emptyCondition",
	// Example suffix patterns (common naming conventions)
	"_example",
	"_simple",
	"_advanced",
}
...
// SnakeCaseToTitleCaseExported is the exported version of toTitleCase for testing
func SnakeCaseToTitleCaseExported(s string) string {
	return toTitleCase(s)
}

// ExtractResourceNameExported is the exported version of extractResourceName for testing
func ExtractResourceNameExported(typeName string) string {
	return extractResourceName(typeName)
}

// ShouldExcludeFileExported is the exported version of shouldExcludeFile for testing
func ShouldExcludeFileExported(filePath string, excludePaths []string) bool {
...
// TestFilePattern represents a parsed test file naming pattern.
type TestFilePattern struct {
	Pattern      string // The prefix or suffix to match
	IsDataSource bool   // Whether this pattern indicates a data source
	IsPrefix     bool   // True for prefix patterns, false for suffix patterns
}

// DefaultTestFilePrefixPatterns returns the default prefix patterns for test file matching.
// Format: "prefix:is_datasource" where is_datasource is "true" or "false".
func DefaultTestFilePrefixPatterns() []TestFilePattern {
	return []TestFilePattern{
		{Pattern: "resource_", IsDataSource: false, IsPrefix: true},
		{Pattern: "data_source_", IsDataSource: true, IsPrefix: true},
		{Pattern: "ephemeral_", IsDataSource: false, IsPrefix: true},
		{Pattern: "action_", IsDataSource: false, IsPrefix: true},
		{Pattern: "iam_", IsDataSource: false, IsPrefix: true},
	}
}

// DefaultTestFileSuffixPatterns returns the default suffix patterns for test file matching.
func DefaultTestFileSuffixPatterns() []TestFilePattern {
	return []TestFilePattern{
		{Pattern: "_resource", IsDataSource: false, IsPrefix: false},
		{Pattern: "_data_source", IsDataSource: true, IsPrefix: false},
		{Pattern: "_datasource", IsDataSource: true, IsPrefix: false},
		{Pattern: "_action", IsDataSource: false, IsPrefix: false},
	}
}

// DefaultTestFileSuffixStrip returns the default suffixes to strip from extracted names.
func DefaultTestFileSuffixStrip() []string {
	return []string{"_generated", "_gen"}
}

// ParseTestFilePatterns parses pattern strings in "pattern:is_datasource" format.
func ParseTestFilePatterns(patterns []string, isPrefix bool) []TestFilePattern {
	var result []TestFilePattern
	for _, p := range patterns {
		parts := strings.SplitN(p, ":", 2)
		if len(parts) != 2 {
			continue
		}
		result = append(result, TestFilePattern{
			Pattern:      parts[0],
			IsDataSource: parts[1] == "true",
			IsPrefix:     isPrefix,
		})
	}
	return result
...
// Uses default patterns; for custom patterns use ExtractResourceNameFromPathWithPatterns.
//
...
func ExtractResourceNameFromPath(filePath string) (string, bool) {
	return ExtractResourceNameFromPathWithPatterns(
		filePath,
		DefaultTestFilePrefixPatterns(),
		DefaultTestFileSuffixPatterns(),
		DefaultTestFileSuffixStrip(),
	)
}

// ExtractResourceNameFromPathWithPatterns extracts a resource name using custom patterns.
// This allows configuration-driven pattern matching for different provider conventions.
func ExtractResourceNameFromPathWithPatterns(filePath string, prefixPatterns, suffixPatterns []TestFilePattern, suffixStrip []string) (string, bool) {
	...
	// Check prefix patterns (in order - more specific patterns should be first in config)
	for _, pattern := range prefixPatterns {
		if strings.HasPrefix(nameWithoutTest, pattern.Pattern) {
			name := strings.TrimPrefix(nameWithoutTest, pattern.Pattern)
			// Apply suffix stripping (e.g., _generated, _gen)
			for _, strip := range suffixStrip {
				name = strings.TrimSuffix(name, strip)
			}
			if name != "" {
				return name, pattern.IsDataSource
			}
		}
	}
	...
	for _, pattern := range suffixPatterns {
		if strings.HasSuffix(nameWithoutTest, pattern.Pattern) {
			name := strings.TrimSuffix(nameWithoutTest, pattern.Pattern)
			if name != "" {
				return name, pattern.IsDataSource
			}
		}
